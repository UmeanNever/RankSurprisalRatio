<p align="center">
 ...
 <p align="center"> From the ACL 2026 paper <strong>"Which Reasoning Trajectories Teach Students to Reason Better? A Simple Metric of Informative Alignment"</strong> </p>
</p>
...
- 🛠️ **Code**: Available in this repository, providing **a clean and efficient implementation for computing the RSR metric**.
  - Update (2026-03-13): Added support for multi-round chat messages, agent data, and multimodal inputs!
- 📀 **Data**: Available on [Hugging Face](https://huggingface.co/datasets/Umean/RSR_data). We release **33 math reasoning trajectory datasets** used in our experiments, generated by 11 different teacher models. We also release RSR-selected datasets for 5 student models. These datasets can be selectively used for reasoning distillation.
...
- `rsr_launch.py` serves as the **entry-point** script and batch runner, calling `rsr_cal.py` with your customized configuration. See inline comments for details. By default, the script processes data in the standard Hugging Face chat message format (now support for multimodal messages). You may refer to our provided datasets on Hugging Face for examples of the expected data format.
- `rsr_cal.py` implements the core computation logic. The code runs a forward pass with the provided model and data, then computes the RSR metric. It includes placeholders and explanatory comments to facilitate easy customization.
- `rsr_utils.py` provides utility functions.
